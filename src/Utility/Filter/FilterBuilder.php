...
class FilterBuilder
{
    ...
    public function withLimit(int $limit): self
    {
        if ($this->maxLimit < $limit) {
            throw new FilterLimitException(sprintf(
    ...
    public function addCriteria(string $key, string $operator, string $value): self
    {
        switch (strtolower($operator)) {
            case Criterion::EQ: return $this->equal($key, $value);
            case Criterion::NOT: return $this->notEqual($key, $value);
            case Criterion::CONTAINS: return $this->contains($key, $value);
            case Criterion::STARTS: return $this->startsWith($key, $value);
            case Criterion::ENDS: return $this->endsWith($key, $value);
            case Criterion::GT: return $this->greaterThan($key, $value);
            case Criterion::GTE: return $this->greaterThanOrEqualTo($key, $value);
            case Criterion::LT: return $this->lessThan($key, $value);
            case Criterion::LTE: return $this->lessThanOrEqualTo($key, $value);
            case Criterion::BETWEEN: return $this->between($key, $value);
            case Criterion::IN: return $this->in($key, explode(Criterion::DELIMITER, $value));
            case Criterion::NIN: return $this->notIn($key, explode(Criterion::DELIMITER, $value));
            case Criterion::EMPTY: return $this->empty($key);
            case Criterion::NOT_EMPTY: return $this->notEmpty($key);
            default:
